const APP_RELEASE=Object.freeze({version:'v0.3.5',buildId:'2026-09-19.2',channel:'admin1-stage',dbSchema:5,updateStrategy:'manifest-service-worker',rolloutStage:'admin1',previousBuildId:'2026-09-19.1'});window.APP_RELEASE=APP_RELEASE;
...
async function manualRefreshData(){if(!backendSession()){alert('Сначала подключите и активируйте устройство.');return {ok:false,error:'NO_SESSION'}}return withBusy('Обновляю доступ и данные…',async()=>{const d=await refreshBackendData();if(document.getElementById('sync')?.classList.contains('active'))await renderSync();alert(d?.ok?'Доступ подтверждён, данные обновлены.':'Не удалось обновить: '+String(d?.error||'ошибка'));return d})}
...
function heavyDraft(x){return !!(x?.blob instanceof Blob)||['photo','audio','document'].includes(String(x?.kind||''))}
...
async function draftToEvent(x){const ev={event_id:x.id,event_type:x.kind||'unknown',kind:x.kind||'unknown',created_at:x.createdAt||'',holdUntil:x.holdUntil||'',sendNowRequested:!!x.meta?.sendNowRequested,context:x.context||'',entity_id:x.objectId||'',objectId:x.objectId||'',text:x.text||'',amount:x.meta?.amount??'',category:x.meta?.type||'',meta:{...(x.meta||{}),appName:x.appName||'',originalName:x.originalName||''}};if(x.blob instanceof Blob){if(x.blob.size>7*1024*1024)throw new Error('Файл больше 7 МБ: '+(x.appName||x.text||x.id));ev.media={base64:await blobToBase64(x.blob),mimeType:x.mime||x.blob.type||'application/octet-stream',fileName:x.appName||x.originalName||('PROD_'+x.id)}}return ev}
...
function go(id){document.querySelectorAll('.screen').forEach(x=>x.classList.remove('active'));document.getElementById(id).classList.add('active');document.getElementById('pageTitle').textContent=titles[id]||'Производство';document.querySelectorAll('.nav').forEach(x=>x.classList.toggle('active',x.dataset.screen===id));window.scrollTo({top:0,behavior:'smooth'});if(id==='home')renderHome();if(id==='checklists')renderChecklists();if(id==='sync')renderSync();if(id==='wallet')renderWallet();if(id==='nom')renderNom();if(id==='avito')renderAvito();if(id==='gallery')renderGallery();if(id==='admin')renderAdmin();if(id==='activityLog')renderActivityLog();if(id==='analytics')renderAnalytics();if(id==='calculator')renderCalculator();if(id==='salesAnalytics')renderSalesAnalytics();}
...
function galleryServerTile(g,i){const key=`server:${i}`;return `<div class="gallery-tile"><img src="${g.image}" alt="${esc(g.title||'готовое изделие')}"><div><b>${esc(g.title||'Готовое изделие')}</b><br><span class="muted">${esc(g.orderId||'')}</span>${galleryPubChips(key)}</div><div class="gallery-tile-actions">${isAdmin1()?`<button class="mini-danger" onclick="alert('Это фото уже находится в серверной Галерее. Реальное удаление будет доступно после backend и проверки права gallery.delete.')">Удалить</button>`:''}</div></div>`}
function galleryLocalTile(g){const key=`local:${g.id}`;return `<div class="gallery-tile"><img src="${blobUrl(g.blob)}" alt="готовое изделие"><div><b>${esc(g.objectId||'Готовое изделие')}</b><br><span class="muted">локально одобрено</span>${galleryPubChips(key)}</div><div class="gallery-tile-actions">${isAdmin1()?`<button class="mini-danger" onclick="deleteGalleryItem('${esc(g.id)}')">Удалить</button>`:''}</div></div>`}
function galleryCandidateItem(x){const media=x.blob instanceof Blob?`<div class="queue-media"><img class="queue-img" src="${blobUrl(x.blob)}" alt="кандидат"></div>`:'';return `<div class="queue-item"><div class="queue-head"><div><div class="queue-type">Кандидат в Галерею</div><div class="queue-context">${esc(x.objectId||'без заказа')} · ${new Date(x.createdAt).toLocaleString('ru-RU')}</div></div><span class="badge warn">кандидат</span></div>${media}<div class="queue-actions">${isAdmin1()?`<button class="approve-btn" onclick="approveGalleryDraft('${esc(x.id)}')">Одобрить</button>`:''}<button class="danger" onclick="deleteDraft('${esc(x.id)}')">Удалить</button></div></div>`}
async function approveGalleryDraft(id){const db=await openDB();const tx=db.transaction('drafts','readwrite');const store=tx.objectStore('drafts');const req=store.get(id);const rec=await new Promise((res,rej)=>{req.onsuccess=()=>res(req.result);req.onerror=()=>rej(req.error)});if(!rec)return;rec.context='gallery-approved-local';rec.meta={...(rec.meta||{}),approvedLocally:true,approvedAt:new Date().toISOString()};store.put(rec);await new Promise((res,rej)=>{tx.oncomplete=res;tx.onerror=()=>rej(tx.error)});await logActivity('gallery_approved',rec);await refreshPending();await renderGallery()}
async function deleteGalleryItem(id){if(!isAdmin1()){alert('Нет права удалять фото из Галереи.');return}if(!confirm('Удалить это фото из Галереи приложения?'))return;const rec=await getDraft(id);await deleteDraftDirect(id);if(rec)await logActivity('gallery_deleted',rec);await refreshPending();await renderGallery()}
let shareFiles=[];
async function galleryShareModal(){shareFiles=[];const all=await drafts();const serverApproved=S.gallery||[],localApproved=all.filter(x=>x.context==='gallery-approved-local'&&x.kind==='photo');const has=(serverApproved.length+localApproved.length)>0;modal('Поделиться из Галереи',`<div class="hint"><b>Черновик публикации.</b> Фото выбираются только из Галереи приложения. После сохранения у выбранных фото появится статус «черновик» для выбранного канала. После реальной публикации backend сменит его на «опубликовано».</div><div class="field"><label>Фото / скриншоты из Галереи приложения</label><button class="secondary attention-green" style="width:100%" onclick="toggleAppGalleryPicker()">Выбрать фото и скриншоты</button><div id="appGalleryPicker" class="share-approved hidden" style="margin-top:8px">${serverApproved.map((g,i)=>`<label class="share-photo"><input type="checkbox" class="shareApproved" value="server:${i}"><img src="${g.image}" alt="${esc(g.title||'работа')}"><span>${esc(g.title||g.orderId||'Работа')}</span></label>`).join('')}${localApproved.map(g=>`<label class="share-photo"><input type="checkbox" class="shareApproved" value="local:${esc(g.id)}"><img src="${blobUrl(g.blob)}" alt="работа"><span>${esc(g.objectId||g.appName||'Работа')}</span></label>`).join('')}${has?'':'<div class="muted">В Галерее приложения пока нет одобренных фото.</div>'}</div></div><div class="field"><label>Текст публикации</label><textarea id="shareText" placeholder="Текст для публикации / комментарий"></textarea></div><div class="field"><label>Куда подготовить</label><div class="channel-grid"><label><input type="checkbox" class="shareChannel" value="VK"> VK</label><label><input type="checkbox" class="shareChannel" value="SITE"> Сайт</label><label><input type="checkbox" class="shareChannel" value="MAX"> MAX</label><label><input type="checkbox" class="shareChannel" value="INSTAGRAM"> Instagram</label><label><input type="checkbox" class="shareChannel" value="TELEGRAM"> Telegram</label><label><input type="checkbox" class="shareChannel" value="WHATSAPP"> WhatsApp</label></div></div><button class="primary" onclick="saveGalleryShare()">Сохранить черновик публикации</button>`)}
...
function galleryCandidateModal(){modal('Фото готового изделия / Галерея',`<div class="field"><label>К какому заказу относится</label><select id="galleryOrder"><option value="">Старая работа / без заказа</option>${S.orders.map(o=>`<option value="${esc(o.id)}">${esc(o.id)} · ${esc(o.name)}</option>`).join('')}</select></div><div class="hint">Фото сначала попадёт в кандидаты. В публичную Галерею автоматически не публикуется.</div><br><div class="photo-option"><button class="primary" onclick="pickGalleryCameraFromModal()">Камера</button><button class="secondary" onclick="pickGalleryFilesFromModal()">Из галереи телефона</button></div>`)}
function galleryCandidateForOrder(id){const o=S.orders.find(x=>x.id===id);modal('Фото готового изделия',`<div class="hint"><b>${esc(id)}${o?' · '+esc(o.name):''}</b><br>Заказ и изделие уже привязаны автоматически. Можно снять готовое изделие камерой или выбрать готовые фото/скриншоты из телефона. Всё сначала попадёт в кандидаты.</div><br><div class="photo-option"><button class="primary" onclick="pickGalleryCameraForOrder('${id}')">Камера</button><button class="secondary" onclick="pickGalleryFilesForOrder('${id}')">Из галереи телефона</button></div>`)}
function gallerySelectedOrder(){return document.getElementById('galleryOrder')?.value||'GALLERY'}
function pickGalleryCameraFromModal(){const id=gallerySelectedOrder();closeModal();pickPhoto('portfolio-candidate',id)}
function pickGalleryFilesFromModal(){const id=gallerySelectedOrder();closeModal();pickGalleryFiles('portfolio-candidate',id)}
function pickGalleryCameraForOrder(id){closeModal();pickPhoto('portfolio-candidate',id)}
function pickGalleryFilesForOrder(id){closeModal();pickGalleryFiles('portfolio-candidate',id)}
...
function permissionGroups(){return [
 ['Заказы',[['orders.view','Видеть заказы'],['orders.create','Создавать заказ'],['orders.edit','Менять заказ'],['orders.media','Фото / голос / файлы']]],
 ['Закупка',[['purchase.view','Видеть «Закупки по заказам»']]],
 ['Кошелёк',[['wallet.view','Видеть Кошелёк'],['wallet.add','Добавлять доход/расход'],['wallet.edit','Редактировать операции']]],
...
function adminAccessCard(x,i,users){
 ...
 const opts=active.map(u=>`<option value="${esc(u.user_id)}">${esc(u.name||u.user_id)} · ${esc(u.role_label||u.role)}</option>`).join('');
...
async function approveAccessRequest(i){
 ...
 try{await adminCall('admin.access.resolve',{access_request_id:x.request_id,status:'APPROVED',user_id:userId});await loadAdminData(true);alert('Заявка одобрена. Пользователь может нажать «Проверить одобрение» на своём устройстве.');await renderAdmin()}catch(e){alert('Ошибка одобрения: '+String(e?.message||e))}
}
...
async function saveWallet(){const type=document.getElementById('wType').value,amount=document.getElementById('wAmount').value.trim(),objectId=document.getElementById('wOrder').value.trim(),text=document.getElementById('wText').value.trim();await putDraft({kind:'wallet',objectId,text:`${type}: ${amount||'?'} ₽${text?' · '+text:''}`,meta:{type,amount}});closeModal();await refreshPending();renderWallet()}
...
const photoInput=document.getElementById('photoInput');const galleryFileInput=document.getElementById('galleryFileInput');const documentInput=document.getElementById('documentInput');let photoCtx={kind:'general',id:''};let galleryFileCtx={kind:'portfolio-candidate',id:'GALLERY',sourceType:'gallery'};let documentCtx={kind:'purchase',id:'PURCHASE'};function pickPhoto(kind,id){photoCtx={kind,id};photoInput.click()}function pickGalleryFiles(kind,id){galleryFileCtx={kind,id:id||'GALLERY',sourceType:'gallery'};galleryFileInput.click()}function pickScreenshot(kind,id){galleryFileCtx={kind,id:id||'',sourceType:'screenshot'};galleryFileInput.click()}function pickDocument(kind,id){documentCtx={kind,id:id||''};documentInput.click()}function linkedOrderMeta(kind,id){if(kind!=='portfolio-candidate'||!id||id==='GALLERY')return {};const o=S.orders.find(x=>x.id===id);return {orderId:id,orderName:o?.name||'',finishedProduct:true}}
photoInput.addEventListener('change',async e=>{const f=e.target.files?.[0];if(!f)return;const appName=appFileName('photo',photoCtx.id,f);await putDraft({kind:'photo',objectId:photoCtx.id,text:appName,appName,originalName:f.name||'',mime:f.type||'image/*',size:f.size||0,blob:f,context:photoCtx.kind,meta:linkedOrderMeta(photoCtx.kind,photoCtx.id)});e.target.value='';closeModal();await refreshPending();if(photoCtx.kind==='portfolio-candidate')await renderGallery();alert(photoCtx.kind==='portfolio-candidate'?'Фото сохранено как кандидат в Галерею. 5 минут оно защищено от отправки; до одобрения клиентам не показывается.':'Фото сохранено локально. 5 минут оно не уйдёт в сеть: можно изменить, удалить или отправить сейчас.')})
galleryFileInput.addEventListener('change',async e=>{const files=[...(e.target.files||[])];if(!files.length)return;for(const f of files){const appName=appFileName('photo',galleryFileCtx.id,f);await putDraft({kind:'photo',objectId:galleryFileCtx.id,text:appName,appName,originalName:f.name||'',mime:f.type||'image/*',size:f.size||0,blob:f,context:galleryFileCtx.kind,meta:{source:'phone-gallery',captureKind:galleryFileCtx.sourceType||'gallery',...linkedOrderMeta(galleryFileCtx.kind,galleryFileCtx.id)}})}e.target.value='';closeModal();await refreshPending();if(galleryFileCtx.kind==='portfolio-candidate'){await renderGallery();alert(`Добавлено в кандидаты Галереи: ${files.length} фото. Их можно посмотреть, одобрить или удалить до синхронизации.`)}else if(galleryFileCtx.kind==='avito'){await renderAvito();alert(`Добавлено материалов Avito из галереи телефона: ${files.length}.`)}else{alert(`Добавлено файлов: ${files.length}.`)}})
