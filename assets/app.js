const APP_RELEASE=Object.freeze({version:'v0.3.7',buildId:'2026-09-20.1',channel:'admin1-stage',dbSchema:5,updateStrategy:'manifest-service-worker',rolloutStage:'admin1',previousBuildId:'2026-09-19.3'});window.APP_RELEASE=APP_RELEASE;
...
async function saveWallet(){const type=document.getElementById('wType').value,amountRaw=document.getElementById('wAmount').value.trim().replace(',','.'),objectId=document.getElementById('wOrder').value.trim(),comment=document.getElementById('wText').value.trim(),amount=Number(amountRaw);if(!Number.isFinite(amount)||amount<=0){alert('Укажите сумму больше 0.');return}const raw=`${type}: ${amountRaw} ₽${comment?' · '+comment:''}`;await putDraft({kind:'finance-entry',context:'wallet',objectId,text:raw,meta:{type,financeType:type,amount:amount,currency:'RUB',rawText:raw}});closeModal();await refreshPending();renderWallet()}
